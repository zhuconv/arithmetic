...
class NormalizedResidualConnection(torch.nn.Module):
    ...
    def __init__(self, input_dim, cfg_arch, output_dim=None, dropout=0.0):
        ...
        if cfg_arch.norm_scheme == "pre":
            ...
            if cfg_arch.attention.rotary_embedding in ['adape', 'adayarn']:
                self._chosen_forward_impl = self._postnormalization_residual_with_two_position_states
            elif cfg_arch.attention.rotary_embedding == 'adalibi':
                self._chosen_forward_impl = self._postnormalization_residual_with_one_position_states
            else:
    ...
    def _postnormalization_residual_with_two_position_states(self, residual, position_residual, layer, states, position_states, *args, **kwargs):
    ...
    def _postnormalization_residual_with_one_position_states(self, residual, position_residual, layer, states, position_states, *args, **kwargs):
        value_output, pos_output = layer(states, position_states, *args, **kwargs)
        return self.norm(residual + value_output), pos_output
